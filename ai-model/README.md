# goktugGPT — AI Model (`ai-model/`)

> This is the **AI model** component of the larger goktugGPT platform — the custom,
> from-scratch transformer LLM. For the overall project (microservices backend,
> upcoming frontend, full tech stack) see the [root README](../README.md). In
> production the model is served by the `llm-server` service in
> [`../backend/`](../backend/), which loads the weights trained here.
...
```
ai-model/                       # (this directory — part of the goktugGPT monorepo)
│
...
│
├── data/
│   └── train_chat.txt          # Training data
│
├── checkpoints/                # Saved model weights (created after training, git-ignored)
│   ├── tokenizer.json
│   ├── best_model.pt
│   └── final_model.pt
│
